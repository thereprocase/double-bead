...
def main():
    ...
    reader = FontReader(fonts / "FillaprintTab-Regular.ttf")
    draw_rows([("Fillaprint  (set P, kerned)", [kerned("fillaprint"), kerned(LOWER)]),
               ("Fillaprint Tab  (mixed setting: tabular figures, 1 on a 9w cell)", [mixed(FIGURES), mixed("0.25 1/16 3/32 7/8 13/32")]),
               ("Fillaprint Mono  (set M, 12w cells)", [tabular(LOWER[:13]), tabular(LOWER[13:]), tabular(FIGURES)]),
               ("Masonry key labels, set from FillaprintTab-Regular.ttf", [reader.layout('1/4" Jt.'), reader.layout('1/2" - 5/8" dep.'),
